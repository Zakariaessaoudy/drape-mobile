...
class CameraItemApi {
  ...
  Future<CameraItem> createItem(CreateCameraItemRequest request) async {
    try {
      ...
      // =====================================================================
      // AJOUT WEB : sur Flutter Web, imagePath est un blob URL (blob:http://...)
      // MultipartFile.fromPath utilise File() en interne → crash sur Web.
      // On lit les bytes via http.get() puis on utilise fromBytes à la place.
      // Sur mobile, le comportement original est conservé (fromPath).
      // =====================================================================
      if (kIsWeb) {
        final blobResponse = await http.get(Uri.parse(request.imagePath));
        final contentType = MediaType.parse(
          blobResponse.headers['content-type'] ?? 'image/jpeg',
        );
        multipart.files.add(
          http.MultipartFile.fromBytes(
            'image',
            blobResponse.bodyBytes,
            filename: 'upload.${contentType.subtype}',
